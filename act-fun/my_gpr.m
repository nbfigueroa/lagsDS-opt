function [hy,model,Sigma] = my_gpr(X,y,model,epsilon,rbf_var,method)
...
% Test the model    
    ...
    elseif strcmp(method,'gpml')
        %%%%% THIS IS THE DEFAULT USAGE; i.e. using GPML toolbox, faster
        %%%%% implementation than the 'manual' one
        meanfunc     = {@meanZero};
        covfunc      = {@covSEiso}; 
        ell          = rbf_var;     % kernel width of RBF covariance function.
        sf           = 1;           % signal variance (not measurement noise)
