...
est_labels_init =  my_gmm_cluster(Xi_ref, Priors, Mu, Sigma, 'hard', []);

% Re-estimate parameters
re_estimate = 0;
if re_estimate
    [Priors, Mu, Sigma, ~] = my_gmmEM(Xi_ref, est_K, 'full', Priors, Mu, Sigma, 100);
end
est_labels = est_labels_init;
...
switch est_options.type   
    ...
    case 1
        title('Best fit GMM with BIC Model Selection','Interpreter','LaTex', 'FontSize',15);
end
...
switch act_type
    ...
    case 1 % With GPR
        ...
        epsilon = 0.001; rbf_var = 0.15; % assuming output variance = 1
...
switch Ag_type
    ...
    case 1 % 1: LPV system sum_{k=1}^{K}h_k(xi)(A_kxi + b_k)            
        ...
        if constr_type == 3
            [A_g, b_g, P_est] = optimize_globalDS_multi_from_data(Data, att_g, constr_type, ds_gmm, P_g, P_l);
        else
             [A_g, b_g, P_est] = optimize_globalDS_multi_from_data(Data, att_g, constr_type, ds_gmm, P_g);
        end
...
ds_types = [3 1 1 3 3]
tracking_factor = [5 15 10 10 10]; % Relative ratio between eigenvalues
A_l = []; b_l = []; A_d = []; b_d = [];
for k=1:est_K
