function [h_dk, ht] = simulation_passive_control_GUI(fig, robot, base, reshaped_ds, target, q, dt, varargin)
handle(fig)
t = 0;
qd = [0,0];
h_dk = [];
...
while ( (get(stop_btn, 'UserData') == 1))
    ...
    if (i == 1) || (mod(i,mod_step) == 0)
        if nargin == 8
            ...
            h_dk = [h_dk, ml_plot_gmm_contour(gca,1,x,D_scaled,color(1,:),1)];
            ...
            h_dk = [h_dk, ml_plot_gmm_contour(gca,1,x,K_scaled,color(2,:),1)];
